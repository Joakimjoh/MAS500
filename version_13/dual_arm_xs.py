...
class InterbotixArmXSInterface:
    ...
    def max_reach_outline(self, num_samples: int = 1000) -> List[List[float]]:
        """
        Approximate the reachable boundary using random sampling of joint configurations.

        :param num_samples: Number of random configurations to sample
        :return: List of [x, y, z] points along outer boundary
        """
        ...
        for _ in range(num_samples):
            config = np.random.uniform(lower, upper)
