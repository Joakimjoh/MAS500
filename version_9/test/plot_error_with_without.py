...
for x, y, depth in region_data:
    if depth > 0:
        if y == center_y:  # Pixels along the x-axis
            ...
            world_z_x.append(point_x[2])  # Real z value adjusted by error
...
plt.ylabel('Z Error', fontsize=18)
plt.title('Depth (Z) Error vs X in World Coordinates (X-Axis)', fontsize=20)
...
plt.ylabel('Z Error', fontsize=18)
plt.title('Depth (Z) Error vs Y in World Coordinates (Y-Axis)', fontsize=20)
plt.legend(fontsize=16)
plt.tick_params(axis='both', which='major', labelsize=14)
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
