...
class InterbotixArmXSInterface:
    ...
    def max_reach_outline(self, resolution_per_joint: int = 5) -> List[List[float]]:
        """
        Estimate the boundary points of the robot's maximum reach in the XY-plane by sampling joint configurations.

        :param resolution_per_joint: Number of samples per joint (higher = more accurate)
        :return: List of [x, y, z] points along the outer boundary of reachable space
        """
        joint_ranges = [
            np.linspace(lo, hi, resolution_per_joint)
            for lo, hi in zip(
                self.group_info.joint_lower_limits,
                self.group_info.joint_upper_limits
            )
        ]

        boundary_points = []

        def recurse(joint_index=0, config=[]):
            if joint_index == len(joint_ranges):
                T = mr.FKinSpace(self.robot_des.M, self.robot_des.Slist, config)
                x, y, z = T[0, 3], T[1, 3], T[2, 3]
                boundary_points.append([x, y, z])
                return
            for angle in joint_ranges[joint_index]:
                recurse(joint_index + 1, config + [angle])
